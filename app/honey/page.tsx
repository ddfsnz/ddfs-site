...
export default async function Page({
    ...
    if (filters.style) {
        searchFilter += ` && honeyOptions.style._ref == "${filters.style}"`;
    }
    if (filters.size) {
        searchFilter += ` && honeyOptions.size.value == ${filters.size}`;
    }

    const sortOrder = filters.sort || "name asc";

    const honeys = await sanity.fetch<Honey[]>(
        `*[_type == "product" && category._ref == "${HONEY_CATEGORY_ID}" ${searchFilter}] | order(${sortOrder}) {
            ...,
            company->{
                ...,
                name
    ...
    const companies = await sanity.fetch<Company[]>(
        `*[_type == "company" && category._ref == "${HONEY_CATEGORY_ID}"] | order(name asc)`,
    );

    const styles = await sanity.fetch<Style[]>(
        `*[_type == "tag" && type == "style" && category._ref == "${HONEY_CATEGORY_ID}"] | order(name asc)`,
    );

    return (
        <main className="relative">
            <div className="absolute inset-0 aspect-[2/1] bg-[url('/hero-honey.png')] bg-contain bg-no-repeat"></div>
            ...
            <div className="relative z-10 mx-auto max-w-7xl px-4 py-32 pt-40">
                <CatalogHeader
                    ...
                    subheading="Experience pure New Zealand Manuka honey, prized for its unique taste and health benefits."
                />
                <ProductCatalog products={honeys} search={filters.search}>
                    <FilterInput
                        label="Producer"
                        filterName="company"
